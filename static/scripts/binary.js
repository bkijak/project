...
function displayBinary(binary1, binary2, carry, answer, column, finished) {
  ...
  var ctd = "</td>"
    ...
    for(var i = 0; i < 8; i++) {
      if(i != column) {
        row1 += td1 + binary1[i] + ctd;
        row2 += td1 + binary2[i] + ctd;
        if(carry[i] == 'empty') {
          row3 += td1 + ctd;
        }
        else {
          row3 += td1 + carry[i] + ctd;
        }
        if(answer[i] == 'empty') {
          row4 += td1 + ctd;
        }
        else {
          row4 += td1 + answer[i] + ctd;
        }
      }
      else if(i == column) {
        //if the loop in the function calling this function has not finished
        if(!finished) {
          row1 += td2 + binary1[i] + ctd;
          row2 += td2 + binary2[i] + ctd;
          if(carry[i] == 'empty') {
            row3 += td2 + ctd;
          }
          else {
            row3 += td2 + carry[i] + ctd;
          }
          if(answer[i] == 'empty') {
            row4 += td2 + ctd;
          }
          else {
            row4 += td2 + answer[i] + ctd;
          }
        }
